<template>
  <div class="jazznpop-text p-page">
    <form enctype="multipart/form-data" @submit.prevent="createArtisteInt()">
      <div class="py-5">
        <h2 class="jazznpop-h2">Créer un artiste international</h2>
      </div>

      <div class="flex w-full flex-col gap-4">
        <div class="flex flex-col">
          <span class="">Nom de l'artiste</span>
          <input class="jazznpop-input" placeholder="Melody Gardot" v-model="artisteInt.nom" required />
        </div>
        <div class="flex flex-col">
          <span>Photo</span>
        ...
        <div class="flex flex-col">
          ...
          <input type="text" class="jazznpop-input" placeholder="jour(XX) mois année(XXXX)" required v-model="artisteInt.date" />
        </div>

        <div class="flex flex-col">
          <span>Catégorie</span>

          <select class="jazznpop-input" v-model="artisteInt.cat">
...
export default {
  ...
  methods: {
    ...
    previewImage: function (event) {
      ...
      this.artisteInt.photo = this.file.name;
    ...
    async createArtisteInt() {
      // Obtenir storage Firebase
      const storage = getStorage();
      // Référence de l'image à uploader
      const refStorage = ref(storage, "artisteInt/" + this.artisteInt.photo);
      ...
      await uploadString(refStorage, this.imageData, "data_url").then((snapshot) => {
        ...
        const docRef = addDoc(collection(db, "artisteInt"), this.artisteInt);
      });
